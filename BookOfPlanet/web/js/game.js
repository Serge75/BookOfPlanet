	...
	document.getElementById("screen").onclick = function(){
		if (MODE == ""){
			document.getElementById("start_frame").className = "frames_row hidden";
			if (Math.random() > 0.5) init(); else location = "survey/";
		}
	}
	...
	function init(){
		...
		document.getElementById("progress_frame").className = "frames_row";
		loadAssets();
	}
		...
		if (WINS[0]){
			...
			for (var n = 0; n < WINS.length; n++) if (WINS[n]){
				var W = WINS[n];
						...
						var XXX = 0;
					for (var j = 0; j < 5; j++){
						if (W.hits[j] == 1) XXX += 0.2; else
						if (W.hits[j] == 2) XXX += 2; else
						if (W.hits[j] == 3) XXX += 0.5; else
						if (W.hits[j] == 4) XXX += 1.0; else
						if (W.hits[j] == 5) XXX += 1.5; else
						if (W.hits[j] == 6) XXX += 1.0; else
						if (W.hits[j] == 7) XXX += 0.75;
				...
				context.fillText(Math.round(PRIZE), 470, 255);
			}
